...
def pixel_norm(features, epsilon=1e-8):
    return features * torch.rsqrt(torch.mean(torch.square(features), dim=1, keepdim=True) + epsilon)
    
def scm_modulate(x, shift, scale):
    if shift is None:
        norm_scale = pixel_norm(scale)
        return x * (1 + norm_scale.unsqueeze(1))
    else:
        norm_scale = pixel_norm(scale)
        norm_shift = pixel_norm(shift)
        return x * (1 + norm_scale.unsqueeze(1)) + norm_shift.unsqueeze(1)
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        # self.AdaLoss = scm_AdaLoss(hidden_size)
    ...
    def initialize_weights(self):
        ...
        # Zero-out timestep embedding MLP:
